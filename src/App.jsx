...
function UrgentView({ users, shifts, urgentCalls, currentUser, isAdmin, onCreateUrgent, onOfferUrgent, onResolveUrgent, onCancelUrgent }) {
  ...
  return (
    <div className="vy-view">
      <div className="vy-view-head">
        <h1>Turno Urgente</h1>
        <p>{isAdmin ? "Autorizá los bonos de los llamados urgentes ofrecidos por el personal" : "Cobertura inmediata con bono de incentivo para personal de franco disponible"}</p>
      </div>
      ...
      <div className="vy-swap-list" style={{ marginTop: 18 }}>
        {relevant.length === 0 ? (
          <EmptyState icon={Siren} title="Sin llamados urgentes" hint={isAdmin ? "Cuando el personal reporte un turno urgente, va a aparecer acá para autorizar el bono." : "Cuando se reporte un turno urgente, va a aparecer acá con el bono ofrecido."} />
...
function NotificationsView({ notifications, currentUser, onMarkRead, urgentCalls, onOfferUrgent, onResolveUrgent, courses, completions, onMarkComplete }) {
      ...
      {mine.length === 0 ? (
        ...
        <div className="vy-notif-list">
          {mine.map((n) => {
            ...
            const canOffer = isUrgent && call && currentUser.role !== "admin" && call.status === "buscando" && call.requestedBy !== currentUser.id;
            const canAuthorize = isUrgent && call && currentUser.role === "admin" && call.status === "pendiente_autorizacion";
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {canAuthorize && (
                    <div className="vy-notif-action-group">
                      <button className="vy-btn vy-btn-approve vy-notif-action" onClick={(e) => { e.stopPropagation(); onResolveUrgent(call.id, "autorizado"); }}>
                        <DollarSign size={13} /> Autorizar bono
                      </button>
                      <button className="vy-btn vy-btn-reject vy-notif-action" onClick={(e) => { e.stopPropagation(); onResolveUrgent(call.id, "rechazado"); }}>
                        <X size={13} /> Rechazar
                      </button>
                    </div>
                  )}
          ...
          <main className="vy-content">
            ...
            {view === "notifications" && (
              <NotificationsView
                ...
                urgentCalls={urgentCalls} onOfferUrgent={handleOfferUrgent} onResolveUrgent={handleResolveUrgent}
...
function GlobalStyle() {
  return (
    <style>{`
      ...
      .vy-notif-action-group { display: flex; gap: 8px; margin-top: 8px; }
      .vy-notif-action-group .vy-notif-action { margin-top: 0; }
